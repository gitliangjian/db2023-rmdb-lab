...
void ResetImplicitTransaction(txn_id_t *txn_id) {
        ...
        if (yyparse() == 0) {
            if (ast::parse_tree != nullptr) {
                try {
                    ...
                    // portal
                    bool is_txn_control = plan->tag == T_Transaction_begin || plan->tag == T_Transaction_commit ||
                                          plan->tag == T_Transaction_abort || plan->tag == T_Transaction_rollback;
                    std::shared_ptr<PortalStmt> portalStmt = portal->start(plan, context);
                    portal->run(portalStmt, ql_manager.get(), &txn_id, context);
                    portal->drop();
                    if (is_txn_control &&
                        (plan->tag == T_Transaction_commit || plan->tag == T_Transaction_abort ||
                         plan->tag == T_Transaction_rollback)) {
