   ...
   public:
    ...
    void beginTuple() override {
        ...
        for (auto &idx_col : index_meta_.cols) {
            ...
            if (it == fed_conds_.end()) {
                exact = false;
                break;
            }
            memcpy(key.data() + offset, it->rhs_val.raw->data, idx_col.len);
            offset += idx_col.len;
        }
        if (exact) {
            ...
            while (!scan_->is_end()) {
                ...
                if (eval_conds_on_record(cols_, rec.get(), fed_conds_)) {
                    matched_rids_.push_back(scan_->rid());
                }
                scan_->next();
            }
        }
        while (rid_pos_ < matched_rids_.size()) {
            auto rec = fh_->get_record(matched_rids_[rid_pos_], context_);
            if (eval_conds_on_record(cols_, rec.get(), fed_conds_)) {
