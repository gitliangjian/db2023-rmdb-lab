...
std::shared_ptr<Query> Analyze::do_analyze(std::shared_ptr<ast::TreeNode> parse)
{
    ...
    if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse))
    {
        ...
        if (x->select_items.empty()) {
            ...
            analyze_select_items(query->tables, all_cols, x->select_items, query->select_items, query->cols);
...
void Analyze::analyze_select_items(const std::vector<std::string> &tables, const std::vector<ColMeta> &all_cols,
                                   const std::vector<std::shared_ptr<ast::SelectItem>> &sv_items,
                                   std::vector<SelectItem> &items, std::vector<TabCol> &cols) {
    (void)tables;
    std::map<ast::AggFunc, AggType> agg_map = {
        {ast::AGG_FUNC_NONE, AGG_NONE}, {ast::AGG_FUNC_SUM, AGG_SUM}, {ast::AGG_FUNC_MAX, AGG_MAX},
        {ast::AGG_FUNC_MIN, AGG_MIN}, {ast::AGG_FUNC_COUNT, AGG_COUNT},
    };
    for (auto &sv_item : sv_items) {
        SelectItem item;
        item.agg_type = agg_map.at(sv_item->agg_func);
        item.is_count_star = sv_item->is_count_star;
        item.alias = sv_item->alias;
        if (!sv_item->is_count_star) {
            item.col = check_column(all_cols, TabCol{.tab_name = sv_item->col->tab_name, .col_name = sv_item->col->col_name});
            if (item.alias.empty()) {
                item.alias = item.col.col_name;
            }
