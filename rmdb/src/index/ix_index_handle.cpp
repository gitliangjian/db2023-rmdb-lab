...
void IxNodeHandle::erase_pair(int pos) {
    ...
    if (move_num > 0) {
        memmove(get_key(pos), get_key(pos + 1), move_num * file_hdr->col_tot_len_);
        memmove(get_rid(pos), get_rid(pos + 1), move_num * sizeof(Rid));
    }
    page_hdr->num_key--;
}
...
std::pair<IxNodeHandle *, bool> IxIndexHandle::find_leaf_page(const char *key, Operation operation,
    ...
    IxNodeHandle *node = fetch_node(find_first ? file_hdr_->first_leaf_ : file_hdr_->root_page_);
...
IxNodeHandle *IxIndexHandle::split(IxNodeHandle *node) {
    ...
    int move_num = node->get_size() - split_pos;
...
void IxIndexHandle::insert_into_parent(IxNodeHandle *old_node, const char *key, IxNodeHandle *new_node,
    ...
    if (old_node->is_root_page()) {
        ...
        root->insert_pair(1, key, new_rid);
...
bool IxIndexHandle::coalesce(IxNodeHandle **neighbor_node, IxNodeHandle **node, IxNodeHandle **parent, int index,
    ...
    (*neighbor_node)->insert_pairs(old_size, (*node)->get_key(0), (*node)->get_rid(0), (*node)->get_size());
...
void IxIndexHandle::maintain_parent(IxNodeHandle *node) {
    page_id_t curr_page_no = node->get_page_no();
    ...
    while (parent_page_no != IX_NO_PAGE) {
        ...
        bool propagate = changed && rank == 0;
        curr_page_no = parent->get_page_no();
        parent_page_no = parent->get_parent_page_no();
        memcpy(first_key.data(), parent->get_key(0), file_hdr_->col_tot_len_);
