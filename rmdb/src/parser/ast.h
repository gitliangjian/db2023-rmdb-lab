...
struct SelectItem : public TreeNode {
    ...
    SelectItem(std::shared_ptr<Col> col_, std::string alias_ = "") :
            agg_func(AGG_FUNC_NONE), col(std::move(col_)), is_count_star(false), alias(std::move(alias_)) {}

    SelectItem(AggFunc agg_func_, std::shared_ptr<Col> col_, bool is_count_star_, std::string alias_ = "") :
            agg_func(agg_func_), col(std::move(col_)), is_count_star(is_count_star_), alias(std::move(alias_)) {}
};
...
struct SelectStmt : public TreeNode {
    ...
    SelectStmt(std::vector<std::shared_ptr<SelectItem>> items_,
               std::vector<std::string> tabs_,
               std::vector<std::shared_ptr<BinaryExpr>> conds_,
               std::shared_ptr<OrderBy> order_,
               int limit_ = -1) :
            select_items(std::move(items_)), tabs(std::move(tabs_)), conds(std::move(conds_)),
            order(std::move(order_)), limit(limit_) {
                has_sort = (bool)order;
                for (auto &item : select_items) {
                    if (item->agg_func == AGG_FUNC_NONE && item->col != nullptr) {
