...
void RmFileHandle::insert_record(const Rid& rid, char* buf) {
    ...
    if (!was_full && page_handle.page_hdr->num_records == file_hdr_.num_records_per_page) {
        if (file_hdr_.first_free_page_no == rid.page_no) {
            ...
            for (int page_no = file_hdr_.first_free_page_no; page_no != RM_NO_PAGE;) {
                RmPageHandle curr = fetch_page_handle(page_no);
                if (curr.page_hdr->next_free_page_no == rid.page_no) {
                    curr.page_hdr->next_free_page_no = page_handle.page_hdr->next_free_page_no;
                    buffer_pool_manager_->unpin_page(curr.page->get_page_id(), true);
                    break;
                }
                int next = curr.page_hdr->next_free_page_no;
                buffer_pool_manager_->unpin_page(curr.page->get_page_id(), false);
                page_no = next;
            }
        }
        page_handle.page_hdr->next_free_page_no = RM_NO_PAGE;
