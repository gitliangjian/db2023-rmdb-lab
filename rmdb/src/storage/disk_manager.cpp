...
void DiskManager::write_page(int fd, page_id_t page_no, const char *offset, int num_bytes) {
    ...
    while (written < num_bytes) {
        ssize_t bytes = write(fd, offset + written, num_bytes - written);
        if (bytes < 0) {
            if (errno == EINTR) {
                continue;
            }
...
void DiskManager::read_page(int fd, page_id_t page_no, char *offset, int num_bytes) {
    ...
    while (read_bytes < num_bytes) {
        ssize_t bytes = read(fd, offset + read_bytes, num_bytes - read_bytes);
        if (bytes < 0) {
            if (errno == EINTR) {
                continue;
            }
        ...
        read_bytes += static_cast<int>(bytes);
    }
}
...
void DiskManager::create_file(const std::string &path) {
    ...
    int fd = open(path.c_str(), O_CREAT | O_EXCL | O_RDWR, S_IRUSR | S_IWUSR | S_IRGRP | S_IROTH);
    if (fd < 0) {
        throw UnixError();
    }
    if (close(fd) < 0) {
