...
std::shared_ptr<Plan> Planner::generate_sort_plan(std::shared_ptr<Query> query, std::shared_ptr<Plan> plan)
{
    ...
    if (x->has_sort) {
        for (size_t i = 0; i < x->order->cols.size(); ++i) {
            TabCol sel_col{.tab_name = x->order->cols[i]->tab_name, .col_name = x->order->cols[i]->col_name};
            if (sel_col.tab_name.empty()) {
                std::string found_tab;
                for (auto &col : all_cols) {
                    if (col.name == sel_col.col_name) {
                        if (!found_tab.empty()) {
                            throw AmbiguousColumnError(sel_col.col_name);
                        }
                        found_tab = col.tab_name;
                    }
                }
                if (found_tab.empty()) {
                    throw ColumnNotFoundError(sel_col.col_name);
                }
                sel_col.tab_name = found_tab;
            }
            sort_cols.push_back(sel_col);
            descs.push_back(x->order->orderby_dirs[i] == ast::OrderBy_DESC);
        }
    }
    return std::make_shared<SortPlan>(T_Sort, std::move(plan), sort_cols, descs, x->limit);
}
...
std::shared_ptr<Plan> Planner::generate_select_plan(std::shared_ptr<Query> query, Context *context) {
    ...
    if (query->has_agg) {
        plannerRoot = std::make_shared<AggregatePlan>(T_Aggregate, std::move(plannerRoot), query->select_items);
        std::vector<TabCol> agg_cols;
        for (auto &item : query->select_items) {
            agg_cols.push_back(TabCol{.tab_name = "", .col_name = item.alias});
        }
        return std::make_shared<ProjectionPlan>(T_Projection, std::move(plannerRoot), std::move(agg_cols));
    }
