...
{
    public:
        SortPlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<TabCol> sel_cols, std::vector<bool> is_desc, int limit)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
            sel_cols_ = std::move(sel_cols);
...
{
    public:
        AggregatePlan(PlanTag tag, std::shared_ptr<Plan> subplan, std::vector<SelectItem> items)
        {
            Plan::tag = tag;
            subplan_ = std::move(subplan);
